...
/*this table define circle under one serviceteam*/
...
* in individual service, no workingteam and have only user self play workingteam role
* other team of service is same as none-individual service
*/
...
* role can contain role, task and operation
* task can contain task and operation
*/
...
/*
 * authitem must belong to one service
 * if service_id = -1001, that service_id can use by all service
 * auth_level:top_service or under_service
 * each service must and can only have one root level of authitem
 * visibility means it is public or private, if public, it can be visiable to all service
 * every service must have own set of authitems, if public authitem is used in 
 * service, it will copy new set of authitem to the service
*/
...
(
    ...
    authlevel VARCHAR(50) DEFAULT 'under_service',
...
CREATE TABLE bln_service_application_user
(
    _id INTEGER NOT NULL PRIMARY KEY AUTO_INCREMENT,
    service_id INTEGER NOT NULL,
