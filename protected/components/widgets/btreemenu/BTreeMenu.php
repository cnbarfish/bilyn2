...
class BTreeMenu extends CWidget
{
	/**
	 ...
	 * <li>link: array, optional, the HTML link associated with this node.
	 * <ul><li>url:string, required, the HTML text associated with this node.</li>
	 * <li>htmlOptions: array, additional HTML attributes (see {@link CHtml::link}).
	...
	//	$cs->registerCoreScript('treeview');
		$assetsFolder=Yii::app()->assetManager->publish(
      	Yii::getPathOfAlias('application.components.widgets.btreemenu.assets'));
		$cs->registerScriptFile($assetsFolder.'/jquery.btreemenu.js');
		$options=$this->getClientOptions();
		$options=$options===array()?'{}' : CJavaScript::encode($options);
		$cs->registerScript('Bilyn.BTreeMenu#'.$id,"jQuery(\"#{$id}\").btreemenu($options);");
		//$cs->registerScript('Bilyn.BTreeMenu#'.$id,"jQuery(\"#{$id}\").treeview({'animated':'fast','collapsed':'false'});");
		//$cs->registerScript('Bilyn.BTreeMenu#'.$id,"jQuery.btreemenu();");
		if($this->cssFile===null)
			$cs->registerCssFile($assetsFolder.'/btreemenu.css');
		else if($this->cssFile!==false)
			$cs->registerCssFile($this->cssFile);

		echo CHtml::tag('div',$this->htmlOptions,false,false)."\n";
		echo CHtml::tag('ul',array(),false,false)."\n";
	...
	public static function saveDataAsHtml($data)
	{
		...
		if(is_array($data))
		{
			foreach($data as $node)
			{
				...
				{					
					$link = $node['link'];
					$linkhtmlOptions = isset($link['htmlOptions'])? $link('htmlOptions'): array();
					$nodeHtml= CHtml::link($node['text'],CHtml::normalizeUrl($link),$linkhtmlOptions);					
				}
				else if(isset($node['isTitle'])?$node['isTitle']:false) 
				{					
					$titleCss = isset($node['titleCss'])?$node['titleCss']:"btreemenu title";
					$nodeHtml= CHtml::tag('div',array('class'=>'btreemenu title'),$node['text'],true);
				}
				else//by default, set link url to self
					$nodeHtml= CHtml::link($node['text'],"#",array());				
				
				if(!empty($node['children']))
				{
					if(isset($node['isChildrenOpened']))
					{
						/*$html.="\n<ul>\n";
					...
					$nodeHtml = CHtml::tag('span',array('class'=>"toggle"),"+",true)." ".$nodeHtml;
				}
				
				$html.=CHtml::tag('li',$options,$nodeHtml,false);
